...
const SongCard = (props) => {
  ...
  const secs = duration_ms
    ? ("0" + Math.floor((duration_ms / 1000) % 60)).slice(-2)
    : "00";
  ...
  const handleEdit = (e) => {
    e.preventDefault();
    axiosWithAuth()
      .put(`/api/favorites/${props.song.favorites_id}`, {
        ...props.song,
      ...
      .then((response) => {
        ...
        props.setSavedSongs(
          props.savedSongs.map((saved) => {
            return saved.favorites_id === props.song.favorites_id
  ...
  const handleDelete = (e) => {
    e.preventDefault();
    axiosWithAuth()
      .delete(`/api/favorites/${props.song.favorites_id}`)
      .then((response) => {
        console.log({ response });
        props.setSavedSongs(
          props.savedSongs.filter(
            (saved) => saved.favorites_id !== props.song.favorites_id
          )
        );
      })
      .catch((err) => {
  ...
  const handleAdd = (e) => {
    e.preventDefault();
    axiosWithAuth()
      .post("/api/favorites", props.song)
      .then((response) => {
        console.log({ response }, "handleAdd Works");
        props.setSavedSongs([...props.savedSongs, response.data]);
      })
      .catch((err) => {
        console.log({ err });
  ...
  return (
    <div className="song-card" style={{color: "#fff"}}>
      {
        <div className="song-card-container">
          <div className="song-card-wrapper">
            ...
            <div className="favorites-duration">
              {mins}:{secs}
            </div>
          </div>
          {togglePanel && (
            <div className="song-visual" onClick={toggle}>
      ...
      {isSearching && (
        <button className="songButton" onClick={handleAdd}>
          ADD
        </button>
      )}
